...
type alias GaloisField =
  { exponents : Array Int, logarithms : Array Int }
...
exponentAtIndex index =
  if index == 0 then
    1
  else if index < logarithmsCount - 1 then
    -- 0 < i < 255
    List.foldl
      (\index ->
        \accumulator ->
          let
            accumulator =
              accumulator `shiftLeft` 1
          in
            if accumulator >= logarithmsCount then
              accumulator `Bitwise.xor` 285
              -- 285
            else
              accumulator
      )
